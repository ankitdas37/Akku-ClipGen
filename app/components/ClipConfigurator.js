...
function fmtTimecode(secs) {
  ...
  const ms = Math.floor((secs % 1) * 100); // 2 decimal places
...
export default function ClipConfigurator({ videoInfo, onGenerate, onRemove, isGenerating }) {
  ...
  useEffect(() => {
    if (videoInfo?.file) {
      ...
      setPreviewSrc(url);
      // Force the browser to load the new Blob URL
      if (videoRef.current) {
                ...
                <div key={seg.id} className="custom-segment-row">
                  <div className="custom-segment-inputs">
                    <div className="custom-segment-field">
                      ...
                      <input type="range" min="0" max={totalDuration} step="0.01" value={seg.start} onChange={e => {
                        const newSegs = [...customSegments];
                        newSegs[i].start = e.target.value;
                        setCustomSegments(newSegs);
                      }} disabled={isGenerating} className="custom-range-slider" />
                      <input type="number" min="0" step="0.01" value={seg.start} onChange={e => {
                        const newSegs = [...customSegments];
                        newSegs[i].start = e.target.value;
                        setCustomSegments(newSegs);
                      }} disabled={isGenerating} style={{ marginTop: '8px' }} />
                    </div>
                    <div className="custom-segment-field">
                      <label>End Time (sec)</label>
                      <input type="range" min="0" max={totalDuration} step="0.01" value={seg.end} onChange={e => {
                        const newSegs = [...customSegments];
                        newSegs[i].end = e.target.value;
                        setCustomSegments(newSegs);
                      }} disabled={isGenerating} className="custom-range-slider" />
                      <input type="number" min="0" step="0.01" value={seg.end} onChange={e => {
                        const newSegs = [...customSegments];
                        newSegs[i].end = e.target.value;
                        setCustomSegments(newSegs);
                      }} disabled={isGenerating} style={{ marginTop: '8px' }} />
